...
def getitem(v,k):
    ...
    assert k in v.D
    keys = list(v.f.keys())
    values = list(v.f.values())
    #print(keys,values)
    #print(k)
    for letter in keys:
        if letter == k:
            num = keys.index(letter)
            return values[num]
    else:
        return 0
...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    values_u, values_v, domain  = u.f, v.f, list(v.D)
    isSame = True
...
def add(u,v):
    ...
    values_u, values_v, domain  = u.f, v.f, list(v.D)
    keys_u , keys_v = list(u.f.keys()) , list(v.f.keys())
    keys = keys_u.copy()
    dic = {}
    for k in keys_v:
        if k not in keys:
...
def scalar_mul(v, alpha):
    ...
    """
    domain = list(v.D)
    vecrange, rangevalues = [], []
    for value in domain:
        if value in v.f:
            negative = alpha*(v.f[value])
            rangevalues.append(negative)
            vecrange.append(value)
    vec_f= dict(zip(vecrange,rangevalues))
    return Vec(v.D,vec_f)
